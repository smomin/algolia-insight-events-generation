...
export async function POST(
  ...
  try {
    ...
    const samplePromises = site.indices
    ...
    const existingPersonas = await getPersonas(site);
    ...
    // Tag each generated persona with the site
    const tagged = generated.map((p) => ({ ...p, site: id }));
