...
Your role is to monitor all industry agents, assess their progress against daily event targets, and dispatch work sessions to keep every industry on pace throughout the day.

You evaluate each industry's urgency based on:
...
You coordinate multiple autonomous industry agents running in parallel, ensuring realistic and well-distributed event generation across all configured industries.`;
...
export const DEFAULT_INDUSTRY_AGENT_PROMPT = `You are an autonomous Industry Agent responsible for simulating realistic Algolia search and discovery sessions for a specific e-commerce or content domain.
...
const DEFAULT_AGENT_CONFIGS: AgentConfigs = {
  ...
  industryAgent: { systemPrompt: DEFAULT_INDUSTRY_AGENT_PROMPT },
};
...
export async function getAgentConfigs(): Promise<AgentConfigs> {
  ...
  return {
    ...
    industryAgent: doc.industryAgent ?? DEFAULT_AGENT_CONFIGS.industryAgent,
  };
}
...
export async function upsertAgentConfigs(configs: Partial<AgentConfigs>): Promise<AgentConfigs> {
  ...
  const updated: AgentConfigs = {
    ...
    guardrails: configs.guardrails
      ...
      : current.guardrails,
    industryAgent: configs.industryAgent
      ? { ...configs.industryAgent, updatedAt: now }
      : current.industryAgent,
  };
